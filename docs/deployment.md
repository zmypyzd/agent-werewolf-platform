...
| `permission denied for table werewolf_*` at runtime | `service_role` is missing GRANTs on the werewolf tables. Apply the full werewolf migration set via `supabase db push` — `20260508030000_werewolf_service_role_grants.sql` plus `20260510000000_werewolf_public_view_regrant.sql` and `20260510010000_werewolf_service_role_regrant.sql` (the second adds `werewolf_matches_public`/`werewolf_replay_events`/`werewolf_decision_traces`/`werewolf_seats`, which the original 20260508030000 file did not include). |
| `GET /api/v1/werewolf-matches` returns 503 with `apply pending Supabase migrations` | Same root cause — apply all three werewolf migrations above. The 503 is an upgrade from the previous opaque 500: `rewrapPostgresError` (`packages/persistence/src/postgres/postgres-werewolf-match-artifact-store.ts`) maps Postgres `42501 insufficient_privilege` to a typed `ServiceUnavailableError`. Verify with `has_table_privilege('service_role', 'public.werewolf_matches_public', 'SELECT')` in `supabase db query`. |
| `GET /api/v1/docs/werewolf-agent-guide` returns 404 in production but works locally | The Dockerfile must `COPY docs/werewolf-http-agent-guide.md /app/docs/...`. The route at `apps/api/src/routes/werewolf-docs.ts:22` resolves the doc path relative to `dist/routes/`; without the `COPY`, the doc isn't bundled into the runner image. Check `Dockerfile` and `.dockerignore` for the explicit re-include pattern (`!docs/werewolf-http-agent-guide.md` after the broad `*.md` exclusion). |
| Render `pnpm build` fails on `tsc -b` after multiple PRs land green individually | Each PR's preview CI builds in isolation; cross-package type imports that compile when one PR is the only change can break when two PRs land that adjust adjacent packages' export surfaces. Search the failing test file for `import type { … } from '@agent-poker/<x>'` where `<x>` doesn't actually export the symbol — most often the type lives in `@agent-poker/shared` and got incorrectly imported from `@agent-poker/persistence` because the consumer package was the test author's mental model. Fix is one-line; the standing followup is to add a workspace-level `pnpm build` smoke step on every PR (currently CI is Vercel-preview only). |
